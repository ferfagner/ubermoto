...
public class BDSimulado {
	...
	// buscar viagem de um passageiro
	public int getQtdeViagens(String passageiro) {
		int resultado = 0;

		for (int i = 0; i < this.passageiros.size(); i++) {
			if (this.passageiros.get(i).getNome().equals(passageiro)) {
				resultado++;
			}
		}
		return resultado;
	...
	// retornar lista de todas as UFs
	ArrayList<String> listaUFsViagem(String passageiro) {
		ArrayList<String> resultado = new ArrayList<String>();
		for (int i = 0; i < this.passageiros.size(); i++) {
			if (this.passageiros.get(i).getNome().equals(passageiros)) {
				resultado.add(this.passageiros.get(i).getUF());
			}
		}
		return resultado;
	}
